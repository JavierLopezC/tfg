...
\begin{itemize}
	\item \textbf{birthRate}: la \gls{tasaNac} correspondiente a cada \gls{genotype}, es decir, es un número que determina la probabilidad que tiene una célula con este \gls{genotype} de reproducirse, y por tanto aumentar su número total en 1 en un ciclo de tiempo.
	\item \textbf{deathRate}: la \gls{tasaDef} correspondiente a cada \gls{genotype}, es decir, es un número que determina la probabilidad que tiene una célula con este \gls{genotype} de morir, y por tanto reducir su número total en 1 en un ciclo de tiempo.
	\item \textbf{mutationRate}: la \gls{tasaMut} correspondiente a cada \gls{genotype}, es decir, es un número que determina la probabilidad que tiene una célula con este \gls{genotype} de mutar, y por tanto variar alguno de sus \glspl{gen}, pasando de ser de este \gls{genotype} a ser de otro \gls{genotype} distinto en 1 en un ciclo de tiempo.
	\item \textbf{N}: número total de células en la simulación, teniendo en cuenta todos los \glspl{genotype} existentes.
	\item \textbf{n\_x}: número de células de un determinado \gls{genotype} concreto, por ejemplo, \textbf{n\_A} será el número de células del \gls{genotype} A, y \textbf{n\_A\_B} las del \gls{genotype} A\_B.
