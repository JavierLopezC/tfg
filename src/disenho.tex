...
\begin{itemize}
	...
	\item Cada sampleEvery:
	\begin{itemize}
		\item para cada regla:
		\begin{itemize}
			\item Si se cumple la Condition:
				\begin{itemize}
					\item Modificamos el valor de las variables de acuerdo al Action		
				\end{itemize}
			\item Se pasa a la siguiente regla	
		\end{itemize}		
	\end{itemize}		
\end{itemize}
...
\begin{itemize}
	\item Se modifica la estructura de salida de la función OncoSimulIndiv en R añadiendo los campos:
	\begin{itemize}
		\item x\$other\$userVarNames lista con los nombres ("ids") de las variables de usuario creadas
		\item x\$other\$userVarValues matriz con los valores de las variables de usuario en cada sampleEvery (en el mismo orden que los nombres de las variables en x\$other\$userVarNames	
	\end{itemize}
	\item Finalidad: poder emplear las variables de usuario (que son arbitrariamente complejas) como datos de salida y no solo como valores a emplear en otros procesos como las intervenciones. Por ejemplo graficar la evolución en el tiempo de algunas variables de usuario creadas puede ser interesante para evaluar la evolución del tumor en una simulación.	
\end{itemize}
